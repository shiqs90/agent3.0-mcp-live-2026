...
**Why UV, not pip + venv + conda?** One fast tool replaces the juggling act. `uv init` scaffolds a project with its own `pyproject.toml` — the exact Python version and dependency list any teammate's machine can reproduce, the same role `package.json` plays in Node. The real starter project shipped this class, [`my-first-project/`](<../01 - 27 Jun - Python Setup & API Basics/my-first-project/>), is exactly that scaffold — `pyproject.toml`, `uv.lock`, `.python-version`, and a `main.py` doing nothing more than proving the wiring works:
...
The real teaching file, [`Python_foundamentals.py`](<../01 - 27 Jun - Python Setup & API Basics/Python_foundamentals.py>), doesn't teach variables and types as an abstract exercise — every line is captioned with the exact agent concept it becomes later:
...
The live demo, [`currency.py`](<../01 - 27 Jun - Python Setup & API Basics/currency.py>), builds exactly this against a real, free, key-free currency API:
...
## 🛠️ The First Hint of "Tools" — [`functions or tools.py`](<../01 - 27 Jun - Python Setup & API Basics/functions or tools.py>)
...
[`tools.py`](<../01 - 27 Jun - Python Setup & API Basics/tools.py>) then takes this one step further, wrapping both functions in a hand-built `@tool` decorator that times every call and registers it in a `TOOL_REGISTRY` dict — a full year before "tool calling" is ever named, the shape is already sitting on screen:
