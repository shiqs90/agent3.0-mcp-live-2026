...
[`langchain_agents.py`](<../06 - 12 Jul - Introduction to LangChain/langchain_agents.py>) is the first working `create_agent()` call in the repo — a plain mock tool plus a genuinely useful one, a URL fetcher:
...
[`langchain_basics.py`](<../06 - 12 Jul - Introduction to LangChain/langchain_basics.py>) takes the same `fetch_text_from_url` tool and points it at a genuinely non-trivial task: a literary research agent that has to *ground* every claim in tool output, not guess:
...
| File | Covers |
|---|---|
| [`langchain_basics.py`](<../06 - 12 Jul - Introduction to LangChain/langchain_basics.py>) | The Great Gatsby research agent — grounded, checkpointed, `deepagents` previewed |
| [`langchain_agents.py`](<../06 - 12 Jul - Introduction to LangChain/langchain_agents.py>) | `get_weather` + `fetch_text_from_url`, the first working `create_agent()` |
| [`main.py`](<../06 - 12 Jul - Introduction to LangChain/main.py>) | Project entry point |
| [`.example.env`](<../06 - 12 Jul - Introduction to LangChain/.example.env>) | Provider key template — copy to `.env`, never commit the real one |
